...
def mip_model(initial_container_file_path, new_container_file_path, m, h, max_diff, _level_num, M, _alpha, _beta, result_folder_path, ex_idx):
    ...
    for j in range(m):
        for k in range(h-1):
            # Constraint : sequence
            model.add_constraint(sum(all_container_seq[i] * x[i,j,k] for i in range(n)) <= (M * (1 - sum(x[i,j,k+1] for i in range(n)))) + sum(all_container_seq[i] * x[i,j,k+1] for i in range(n)))
            
            # Constraint : relocation by weight considering group
            model.add_constraint(r[j,k] <= (M * (1 - sum(x[i,j,k+1] for i in range(n))))+ r[j,k+1])            
            model.add_constraint((sum((all_container_score[i] * x[i,j,k]) for i in range(n)) - sum((all_container_score[i] * x[i,j,k+1]) for i in range(n)))/ M <= (M * (1- sum(x[i,j,k+1] for i in range(n)))) + r[j,k])
...
def multiple_main():
    ...
    for input_folder_name in input_folder_list:
        ...
        input_folder_path = os.path.join(input_folder, input_folder_name)
        
        container_num = input_folder_name.split('Input_Data_')[1].split('(')[0]
        
        initial_folder_list = os.listdir(input_folder_path)

        for _initial_folder in initial_folder_list:
            
            initial_folder_path = os.path.join(input_folder_path, _initial_folder)

            _new_folder_list = os.listdir(initial_folder_path)
            
            for _new_folder in _new_folder_list:
                
                new_folder_path = os.path.join(initial_folder_path, _new_folder)
                output_folder_path = os.path.join(result_folder, f'{_initial_folder}_Output_Data_{container_num}(stack_{stack_num}_tier_{tier_num})')

                
                print('Input Folder Path : ', new_folder_path)
                print('Output Folder Path : ', output_folder_path)


                initial_file_names, new_file_names, experiment_idx_list = get_input_file(new_folder_path)
                
                experiment_num = len(experiment_idx_list)
                
                if len(initial_file_names) != len(new_file_names):
                    ...
                    for alpha in alpha_list:
                        ...
                        for file_idx in range(experiment_num):
                            
                            experiment_idx = experiment_idx_list[file_idx]
                            
                            initial_file = os.path.join(new_folder_path, f'Initial_state_ex{experiment_idx}.csv')
                            new_file = os.path.join(new_folder_path, f'Container_ex{experiment_idx}.csv')
...
# Single main Parameters
stack_num = 6
tier_num = 5
peak_limit = 2

input_folder = f'Ungrouped_Input_Data_ver0'
result_folder ='Ungrouped_Data\MIP'
...
#             output_folder = os.path.join(result_folder, f'{_initial_folder}_Output_Data_{container_num}(stack_{stack_num}_tier_{tier_num})')
...
# input_folder = f'Ungrouped_Input_Data_ver0/Input_Data_{container_num}(stack_{stack_num}_tier_{tier_num})'
# result_folder ='Ungrouped_Result/MIP'
